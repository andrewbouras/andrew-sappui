...
const ShareModal: React.FC<ShareModalProps> = ({ isOpen, onClose, type, id }) => {
  ...
  const generateShareLink = async () => {
    try {
      ...
      const response = await axios.post(`${apiUrl}/${type}s/${id}/share`, 
        { access: shareAccess },
      ...
      <DialogContent className="sm:max-w-md bg-white dark:bg-gray-800">
        <DialogHeader>
          <DialogTitle className="text-lg font-semibold text-gray-900 dark:text-gray-100">
            Share {type === 'notebook' ? 'Notebook' : 'Chapter'}
          </DialogTitle>
        </DialogHeader>
